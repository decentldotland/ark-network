...
export async function checkAndVerifyUser(userObject) {
  try {
    ...
    if (telegram_username) {
      ...
      console.log(`Telegram identity valid --> ${telegram_username}`);
    }
    ...
    if (validity.is_equal) {
      ...
      const input = `{"function": "verifyIdentity", "identityOf": "${arweave_address}"}`;
      ...
      console.log(
        `-->identity verified: ${arweave_address} <-> ${evm_address}`
      );
      console.log(
